...
- `POST /api/webhooks/doku/notification` — DOKU's server-to-server callback, declared in `routes/api.php` (the `api` group, so no session/CSRF and no exemption needed); no `auth`/`signed` middleware (it's neither a browser session nor a donor-facing link), authenticity enforced by HMAC inside `DokuWebhookController` itself.
